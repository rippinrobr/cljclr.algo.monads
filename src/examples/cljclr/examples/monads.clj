...
(ns
  ...
  examples.monads
  (:use [cljclr.algo.monads           ;RR: clojure.algo.monads
...
(defn rng [seed]
  (let [m      259200
        value  (/ (double seed) (double m))              ;RR: (/ (float seed) (float m))
...
(defn mean [xs]  (/ (double (sum xs)) (double (count xs))))     ;RR: (/ (sum xs) (count xs))
(defn variance [xs]
  (let [m (mean xs)
        sq #(* % %)]
  ...
  (call-cc
   (fn [k]
     (domonad cont-m
       ...
       (str (Math/Sqrt x))))))                            ;RR: (. Math sqrt x)
