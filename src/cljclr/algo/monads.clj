...
;;  Feb 14, 2012
;;  Changes are
;; Copyright (c) Rob Rowe, 2012. All rights reserved.  The use
;; and distribution terms for this software are covered by the Eclipse
;; Public License 1.0 (http://opensource.org/licenses/eclipse-1.0.php)
;; which can be found in the file epl-v10.html at the root of this
;; distribution.  By using this software in any fashion, you are
...
;; Changes to Konrad Hinsen's code are clearly marked.
;; An end-of-line comment start with  ;RR:  indicates a change on that line.

(ns
  ^{:author "Konrad Hinsen, modified for ClojureCLR by Rob Rowe"
     ...
     :doc "This library contains the most commonly used monads as well
           ...
           functions."}
  cljclr.algo.monads                     ;RR: clojure.algo.monads
  (:require [clojure.set])
...
(extend-protocol writer-monad-protocol
  ...
  System.String   ; RR: java.lang.String
...
(defmacro monad-transformer
  ...
  `(let [which-m-plus# (cond (= ~which-m-plus :m-plus-default)
                             ...
                             (or (= ~which-m-plus :m-plus-from-base)
                               ...
                               ~which-m-plus
                             :else
                               (throw (System.ArgumentException.         ; RR: java.lang.IllegalArgumentException
